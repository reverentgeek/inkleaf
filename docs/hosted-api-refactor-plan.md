...
- [ ] Rename `backend/` → `local-agent/` (or keep name to reduce churn — decide up front).
...
- [ ] `.env.example`: split vars — agent (`SQLITE_PATH`, `PORT`, `HOSTED_API_URL`, `SYNC_INTERVAL_MS`) vs server (`MONGODB_URI`, `MONGODB_DB`, `OPENAI_API_KEY`, `JWT_SECRET`, `AUTH_USERNAME`, `AUTH_PASSWORD_HASH`).
