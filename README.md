...
I've been living in Markdown for years. I wanted a personal note app that fully supported Markdown syntax, and an excuse to play around with [Tauri](https://v2.tauri.app/) (JavaScript + Rust alternative to Electron.js for building desktop apps) and [MongoDB Atlas](https://www.mongodb.com/products/platform). So, Inkleaf was born.

> **Note:** Inkleaf is a personal project and a demo, not a product. It's a single-user desktop app built in my spare time, and it's developed on macOS, so Windows and Linux are under-tested. Poke around, borrow what you like, but maybe don't move your life's work into it just yet 😬
...
- **Hybrid search**: one search box, both retrievers. `$rankFusion` runs `$search` and `$vectorSearch` in a single aggregation and merges them with reciprocal rank fusion, so a query finds notes by keyword *and* by meaning
- **Atlas Search**: full-text with fuzzy matching, autocomplete, and highlighted results via the `$search` aggregation stage
- **Atlas Vector Search**: embeddings and `$vectorSearch` power the semantic half of hybrid search, plus a related notes panel. Pick your embedding provider: MongoDB's **Voyage AI** `voyage-4-lite` (the default) or OpenAI `text-embedding-3-small`
- **Offline-first**: every note lives in a local SQLite store, so you can read and write with no connection. Changes sync to Atlas on their own once you're back (last write wins), and search falls back to SQLite FTS5 while you're offline
...
Want the full story? [Saving a note](docs/save-note.md) and [hybrid search](docs/hybrid-search.md) trace a single action all the way through the stack.
...
- [pnpm](https://pnpm.io/)
- A [MongoDB Atlas](https://www.mongodb.com/atlas) cluster running **MongoDB 8.0 or newer**, which is what `$rankFusion` needs. The free M0 tier is plenty for following along. On an older cluster, hybrid search degrades to plain full-text search instead of breaking.
- [Rust](https://www.rust-lang.org/tools/install), but only for the desktop window. See below.
- Optional: an embedding provider API key if you want the semantic half of search, either a MongoDB [Voyage AI](https://www.mongodb.com/products/platform/voyage-ai) key (the default) or an [OpenAI API key](https://platform.openai.com/). Editing, syncing, and keyword search all work fine without one.
...
> **Pro tip:** On macOS you'll also want the Xcode Command Line Tools (`xcode-select --install`).

The first `pnpm dev:tauri` compiles every Tauri crate and takes a few minutes. Coffee break. Later builds take seconds.
...
Open `.env` and add your Atlas connection string, plus an embedding key if you want the semantic half of search:
...
One thing that trips up most people, me included: make sure your current IP address is in the Atlas **Network Access** IP Access List. Otherwise you'll get a connection timeout that looks like a bug in the app.
...
| `HYBRID_TEXT_WEIGHT` | No (default `0.4`) | Weight for the full-text half of `$rankFusion` |
| `HYBRID_VECTOR_WEIGHT` | No (default `0.6`) | Weight for the vector half of `$rankFusion` |
...
That drops in 17 sample notes about MongoDB, React, TypeScript, and friends. If your embedding provider key is set, it generates vector embeddings for each one too, which is what makes hybrid search interesting on a fresh install.

> **Heads up:** seeding an empty database is uneventful. Re-seeding deletes every existing note first, so if the `notes` collection already has data, the script refuses and makes you type `pnpm seed --force`. That guard exists because I once clobbered my own notes.
...
- **Conflicts**: settled per note, newest `updatedAt` wins.
- **Offline search**: search quietly falls back to SQLite FTS5, highlights included. You lose the semantic half, since generating a query embedding means calling an API, but keyword matching keeps working and the UI says so.
...
The [`docs/`](docs/) folder has walkthroughs of how the app actually works today ([saving a note](docs/save-note.md), [hybrid search](docs/hybrid-search.md)), plus a few exploratory sketches of directions Inkleaf *could* go. Those sketches are labeled **possible future ideas**, and none of it is planned, scheduled, or built.

## Contributing

Bug reports and fixes are very welcome. [CONTRIBUTING.md](CONTRIBUTING.md) covers setup, scope, and what to check before opening a pull request. Found a security problem? Please [report it privately](SECURITY.md) instead of filing a public issue. Everyone here follows the [Code of Conduct](CODE_OF_CONDUCT.md).

## More info

- [Tauri v2 docs](https://v2.tauri.app/)
- [MongoDB Atlas Search docs](https://www.mongodb.com/docs/search/)
- [MongoDB Atlas Vector Search docs](https://www.mongodb.com/docs/vector-search/)
